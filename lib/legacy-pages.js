...
export function readLegacyPage(fileName) {
  ...
  const description = html
    ...
    ?.trim();
  const body = addLandingPageFormMount(addImageAnimations(normalizeLegacyLinks(stripSharedShell(stripScripts(extractBody(html))))));
...
export function readLegacyFullPage(fileName) {
  ...
  const description = html
    .match(/<meta\s+name=["']description["']\s+content=["']([^"']*)["']/i)?.[1]
    ?.trim();
  const body = addLandingPageFormMount(addImageAnimations(normalizeLegacyLinks(stripScripts(extractBody(html)))));
